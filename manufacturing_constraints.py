...
def detect_too_thin_solid(pattern, min_width_px):
    ...
    to_remove = np.abs(pattern_binary - narrowed_widened) * pattern
    return to_remove
...
if __name__ == '__main__':
    ...
    x = jax.random.uniform(jax.random.key(42), (16, 16))
    x = jax.image.resize(jnp.pad(x, 1, 'empty'), (n_px, n_px), method='cubic')
    x = soft_binarize(x)
