...
class PatternToTRxyAmpsFNO(nnx.Module):
    def __init__(
            self,
            hidden_n_channels: tuple[int],
            n_pixels: int,
            ...
            relative_lattice_period: float,
            activation_fn: Callable,
            rngs: nnx.Rngs
    ):
        r = relative_lattice_period
        n_propagating_modes = int(np.sum(2 * np.floor(np.sqrt(r ** 2 - np.arange(-int(r), int(r) + 1) ** 2)) + 1))
        self.n_pixels = n_pixels
        self.fno = FourierNeuralOperator(1, 8, hidden_n_channels, n_pixels, mode_threshold, activation_fn, rngs)
        self.propagating_indices = np.array([
            [0, -1, 0, 0, 1, -1, -1, 1, 1, -2, 0, 0, 2, -2, -2, -1, -1, 1, 1, 2, 2, -2, -2, 2, 2, -3, 0, 0, 3, -3, -3,
             -1, -1, 1, 1, 3, 3, -3, -3, -2, -2, 2, 2, 3, 3, -4, 0, 0, 4, -4, -4, -1, -1, 1, 1, 4, 4, -3, -3, 3, 3],
            [0, 0, -1, 1, 0, -1, 1, -1, 1, 0, -2, 2, 0, -1, 1, -2, 2, -2, 2, -1, 1, -2, 2, -2, 2, 0, -3, 3, 0, -1, 1,
             -3, 3, -3, 3, -1, 1, -2, 2, -3, 3, -3, 3, -2, 2, 0, -4, 4, 0, -1, 1, -4, 4, -4, 4, -1, 1, -3, 3, -3, 3]
        ])[:, :n_propagating_modes]
        n, m = self.propagating_indices
        self.kx = n / r
        self.ky = m / r
        self.kz = jnp.sqrt(1 - (n / r) ** 2 - (m / r) ** 2)
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        (
            ...
            (jnp.fft.fft2(field) / (self.n_pixels ** 2))[..., self.propagating_indices[0], self.propagating_indices[1]]
            for field in [trans_ex, trans_ey, ref_ex, ref_ey]
        ]

        trans_exy_amps = jnp.stack([trans_ex_amps, trans_ey_amps], axis=-2)
        ref_exy_amps = jnp.stack([ref_ex_amps, ref_ey_amps], axis=-2)
        amps = jnp.stack([trans_exy_amps, ref_exy_amps], axis=-3)

        trans_ez_amps = (self.kx * trans_ex_amps + self.ky * trans_ey_amps) / self.kz
        ref_ez_amps = (self.kx * ref_ex_amps + self.ky * ref_ey_amps) / self.kz
        total_energy = jnp.sum((
                                       jnp.abs(trans_ex_amps) ** 2 + jnp.abs(trans_ey_amps) ** 2 + jnp.abs(
                                   trans_ez_amps) ** 2 +
                                       jnp.abs(ref_ex_amps) ** 2 + jnp.abs(ref_ey_amps) ** 2 + jnp.abs(ref_ez_amps) ** 2
                               ) * self.kz[None, :], axis=-1)
        amps /= jnp.sqrt(total_energy)[..., None, None, None]
...
if __name__ == '__main__':
    ...
    x = np.load('freeform_training_patterns_4224.npy')[-1]
    # plt.imshow(x)
    # plt.show()

    rngs = nnx.Rngs(42)
    model = PatternToTRxyAmpsFNO(
        hidden_n_channels=[32] * 5,
        n_pixels=100,
        mode_threshold=16,
        relative_lattice_period=2000 / 650,
